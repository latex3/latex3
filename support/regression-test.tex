...
% The luatex package will use the extended pool for all allocations:
% to reduce test file variation, force the same approach for all engines
% even though this is less efficient in pdfTeX/XeTeX
...
% Tests for XeTeX and LuaTeX (and pdfTeX by exclusion)
\begingroup\expandafter\expandafter\expandafter\endgroup
\expandafter\ifx\csname XeTeXversion\endcsame\relax
  \regression@test@alloc {30} \newbox
  \regression@test@alloc {30} \newdimen
  \regression@test@alloc {30} \newskip
  \begingroup\expandafter\expandafter\expandafter\endgroup
  \expandafter\ifx\csname directlua\endcsame\relax
    \regression@test@alloc {30} \newcount
  \else
    \regression@test@alloc {27} \newcount
  \fi
\else
  \regression@test@alloc {31} \newbox
  \regression@test@alloc {37} \newcount
