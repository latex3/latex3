...
%
% Copyright 2015-2026 The LaTeX Project
%
...
\fi
% The data can only be loaded by Unicode engines, which can be detected by
% looking for \Uchar. There are a couple of wrinkles we then need to deal
% with. First, upTeX is not-quite a Unicode engine, so we need to skip it.
% Second, HiTeX doesn't allow setting of the full range (it pre-loads the
% Unicode data), so again we need to bail out.
\ifdefined\Uchar
  \ifdefined\kanjiskip
    \expandafter\expandafter\expandafter\endinput
  \fi
  \ifdefined\HINTversion
    \expandafter\expandafter\expandafter\endinput
  \fi
\else
  \expandafter\endinput
...
% Write some basic information to the log.
  ...
  \newlinechar=`\^^J %
  \message{^^J}%
  \message{load-unicode-data.tex v1.21 (2026-09-25)^^J}%
...
% needs to be updated when a new download takes place!
  \message{\string# UnicodeData-18.0.0.txt^^J}%
  \message{\string# Modified 2026-09-25 07:00 GMT [JAW]^^J}%
