...
compared to their original \texttt{.tlg} file. Directories and file names are adjustable and other setups are possible, the above structure is simply the default.
...
Both  commands  surrounds the generated output with \cs{SEPARATOR}s and 
...
\meta{engine} argument can be given.)  This command can be re-run as
...
The main work script is \texttt{l3build.lua} which is found in the
\texttt{texmf} tree by using \texttt{kpsewhich} and then loading
...
\item It is not necessary to load a particular document class (most
  tests use \texttt{article} or \texttt{minimal}), but a package author
  may wish to adjust page margins, etc., without repeating the
  commands for each test. Such a special test class could then be kept
  in the \texttt{support/} directory.
...
\item The test begins proper at \cs{START}\Dash everything before that point
  ...
  that package version numbers displayed while the preamble is
...
\item A slightly modified version of \verb|\end{document}| finishes
  ...
  used as that would stop the run immediately without producing a page
...
and therby unnecessarily invalidate the test result.
%
You can also use \cs{OMIT} in front of \verb|\end{document}| if you
...
variables that can be set or reset (if necessary) in the driver file.

This allows, for example, to build complex distributions consisting of
several modules in different directories with dependencies between
them. You can also control if the processing should happing in a
sandbox or if it is allowed to draw any support files needed for the
tests (e.g., extra packages or classes) from the TDS tree.
%
For details consult the documentation in~\cite{L3Project:l3build}.

There is one option that one may has to modify even for simple setups:
\texttt{checkruns}. This controls the number of times each test file
is run and to speed up processing it defaults to~\texttt{1}. If,
however, the codes requires multiple runs to function (e.g., if you
...
\execname~check~\meta{test name}
\end{quote}
So here is the list of list of available tasks:
\begin{description}
\item[\texttt{check} \meta{name} \meta{engine}] Without further
  ...
  If supplied with \meta{name} it will only run the tests for that
  \texttt{.lvt} file and if additionally given an \meta{engine} name will
  only run the test for that specific engine. In either case it will
  show everything on the screen which is helpful if the run shows
  upnormal behaviour (especially if it ends up in an endless loop and
  never returns for some reason).

\item[\texttt{clean} ] This cleans up the source tree and removes
  temporary files and directories.

\item[\texttt{ctan} ] Runs all tests, typesets all
  documentation and if there aren't any errors, generates a
  \texttt{.zip} file suitable for uploading to the CTAN archive.

\item[\texttt{doc} ] This typesets all documentation (by default the
  \texttt{.dtx} files checking them for trivial processing errors.
...
\item[\texttt{save} \meta{name} \meta{engine}] This generates (or
  ...
  supplied an \meta{engine} argument it generates an specific
  ...
  actually correct. Once produced or updated in this fashion the
...
\newblock Talk given at TUG conference in Portland. Video and slide material
...
\newblock The \textsf{l3build} package: Checking and building packages,
  September 2014.
\newblock Available at \url{http://www.ctan.org/pkg/l3build}.
