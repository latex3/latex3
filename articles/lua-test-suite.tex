...
\TeX{} programming can be either \emph{expandable} or not.  (\TB's
parlance for these terms is based on whether the processing occurs in
...
The main work script is \texttt{l3build.lua}, which is automatically
found in the \texttt{texmf} tree (by using \texttt{kpsewhich}) and then
...
\else
\SetBibJustification{\small} %\raggedright}
