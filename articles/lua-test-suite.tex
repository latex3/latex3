...
flexibility. This is achieved by providing the large number of
variables that can be set (if necessary) in the driver file.
%
For details consult the documentation in~
\cite{l3build.dtx}.

\textit{FMi a bit more flesh here but avoid a full list of options I think}
...
As inidicated earlier the system does a bit more than managing a set
...
\item[\texttt{save} \meta{name} \meta{engine}] This generates (or
  ...
  placed into the \texttt{.tlg} shows the desired result, i.e., is
  actually correct. Because from that point this output is considered
  certified and will be used in check runs!
...
\bibitem[1]{l3build.dtx} l3build.dtx

\bibitem[2]{label3} regression test TUB article


\bibitem[3]{label4} link to talk?

\bibitem[4]{label5} 
Donald E. Knuth. A torture test for T
E
X. Report
STAN-CS-84-1027, Stanford University, Depart-
ment of Computer Science, Stanford, CA, USA, 1984
