...
\subsection{Behavior of \texttt{c}-type expansion for undefined arguments}

When \texttt{c}-type expansion is applied to with and argument that is
undefined, the result is a token equal to |\scan_stop:| (|\relax|). This will
likely lead to low-level errors if applied to variables. As such, the
programmer should ensure that \texttt{c}-type expansion is only applied where
resulting variable names will definitely exist.
