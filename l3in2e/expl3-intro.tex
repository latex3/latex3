...
have renamed things a little. In this case, |\cs_set:NNn| is what you will be
used to reading as |\newcommand|; we'll get into the reasons behind this change
in section~\ref{sec:funcs}.

What about |\tlist_if_blank:nTF|\,? Well, the meaning seems fairly obvious; if
...
With a macro like |\tlist_reverse:n|, we know simply from the macro name that
it takes a single argument; |\tlist_reverse:n{abc}|, which
expands to |cba|.

Another example is |\tlist_if_eq:nnTF| which expands to the |T| argument if
...
applies here. In fact, he says it's not very useful to include the `type' of
the variable in the name of it, which is exactly what we do for
|int|, |tlp|, and so on. But this is low-level code. When you start writing
higher-level abstractions, such as |clist| and |prop|, it's the semantics
of the variable that's important, and this is what's shown in the variable
name.
...
\textsf{expl3}, the data type tlp (Token Pointer List) is used for
