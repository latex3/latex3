...
functions.  Then Section~\ref{sec:access} discusses some advantages of the
...
\item[Document mark-up] This comprises the commands (often called tags)
  ...
  not pre-empted as primitives (\eg |\box| or |\special|). 
...
\item[c]  Character string used as a command name.\\
  ...
  command name (via~|\csname|, |\endcsname|). This single 
...
\item[N]  Single token (unlike~|n|, the argument must not be
 ...
 \item[X]  Single fully expanded token (unbraced).\\
   ...
   braced token list) to the function.
 \item[C]  Character string used as a command name then one-level expanded.\\
...
|\test_if_in:nn|; thus the variant function we need will be 
...
A large range of similar argument processing functions are available 
